use mem_core::{Context, MemoryLayer, MemoryMetrics, MemoryRole, MindPalaceConfig, TokenCounter};
use std::sync::Arc;
    ...
    pub async fn optimize(&self, context: &mut Context) -> anyhow::Result<()> {
        ...
        if let Some(metrics) = &self.metrics {
            ...
            if let (Some(ref counter), Some(start_tokens)) = (&self.token_counter, initial_tokens) {
                let final_tokens: usize = context
                    .items
                    .iter()
                    .map(|i| counter.count_tokens(&i.content))
                    .sum();
                ...
                tracing::info!(
                    ...
                    final_tokens,
                    if start_tokens > 0 {
                        (final_tokens as f32 / start_tokens as f32) * 100.0
                    } else {
                        100.0
                    }
                );
            ...
            if initial_size > 0 {
                metrics
                    .compression_ratio
                    .set((final_size as f64 / initial_size as f64) * 100.0);
            }
        }
        ...
        if context.items.len() > self.config.max_context_items {
            ...
            if let Some(system_msg) = context
                .items
                .iter()
                .find(|i| i.role == MemoryRole::System)
                .cloned()
            {
            ...
            let target_retain = self
                .config
                .max_context_items
                .saturating_sub(final_items.len());
            ...
            while start_idx > 0 && start_idx < context.items.len() {
                ...
                if current_role == &MemoryRole::Tool {
                    ...
                    let next_is_tool = start_idx + 1 < context.items.len()
                        && context.items[start_idx + 1].role == MemoryRole::Tool;
                    if next_is_tool {
            ...
            while start_idx < context.items.len()
                && context.items[start_idx].role == MemoryRole::Tool
            {
                start_idx += 1;
            }
        ...
        tracing::info!(
            ...
            context.items.len(),
            if initial_size > 0 {
                (final_size as f32 / initial_size as f32) * 100.0
            } else {
                100.0
            }
        );
