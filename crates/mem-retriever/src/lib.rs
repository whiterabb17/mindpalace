...
use mem_core::{
    utils, Context, EmbeddingProvider, FactGraph, FactNode, KnowledgeBase, LlmClient, MemoryItem,
    MemoryRole, StorageBackend,
};
use ruvector_core::index::hnsw::HnswIndex;
use ruvector_core::index::VectorIndex;
pub use ruvector_core::types::{DistanceMetric, HnswConfig, VectorId};
...
pub trait VectorStore: Send + Sync {
    ...
    ) -> anyhow::Result<Vec<(FactNode, f32)>>;
...
impl VectorStore for InMemoryStore {
    ...
    async fn search(
        ...
        let mut candidates: Vec<_> = facts
            .values()
            .filter(|(f, _)| category.is_none() || category.as_deref() == Some(&f.category))
...
impl RuVectorStore {
    ...
    pub fn new(dim: usize, metric: DistanceMetric, graph: Arc<FactGraph>) -> Self {
        ...
        Self {
            index: RwLock::new(
                HnswIndex::new(dim, metric, config).expect("Failed to initialize HnswIndex"),
            ),
            graph,
...
impl<S: StorageBackend> MemoryRetriever<S> {
    ...
    pub async fn append_from_kb(&self, kb_path: &str) -> anyhow::Result<()> {
        if !self.storage.exists(kb_path).await {
            return Ok(());
        }
        ...
        let kb: KnowledgeBase = serde_json::from_slice(&data).unwrap_or_else(|e| {
            tracing::warn!(
                "Failed to parse knowledge base from {}: {}. Starting fresh.",
                kb_path,
                e
            );
    ...
    pub async fn retrieve_relevant_facts(
        ...
        self.store
            .search(
                query_embedding,
                top_k,
                category_filter.map(|s| s.to_string()),
            )
            .await
    }
