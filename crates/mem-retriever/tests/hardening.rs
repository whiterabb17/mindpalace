...
impl EmbeddingProvider for MockEmbeddings {
    async fn embed(&self, _text: &str) -> anyhow::Result<Vec<f32>> { Ok(vec![0.0; 1536]) }
}

struct MockLlm;
#[async_trait]
impl LlmClient for MockLlm {
    async fn completion(&self, _prompt: &str) -> anyhow::Result<String> { Ok("[]".to_string()) }
}
...
async fn test_hydrate_from_empty_kb() {
    ...
    let store = Arc::new(RuVectorStore::new(1536, DistanceMetric::Cosine, graph.clone()));
    let retriever = MemoryRetriever::new(storage.clone(), embeddings, llm, store, graph);

    // Create an empty file
    let kb_path = "knowledge.json";
    tokio::fs::write(dir.path().join(kb_path), b"").await.unwrap();
...
async fn test_hydrate_from_corrupt_kb() {
    ...
    let kb_path = "knowledge.json";
    tokio::fs::write(dir.path().join(kb_path), b"{ invalid json }").await.unwrap();
