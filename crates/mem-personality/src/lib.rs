...
use mem_core::{Context, LlmClient, MemoryItem, MemoryLayer, MemoryRole};
use std::sync::Arc;
...
impl PersonalityGuard {
    ...
    pub async fn check_consistency(&self, context: &mut Context) -> anyhow::Result<()> {
        ...
        let last_assistant_msg = context
            .items
            .iter()
            .rev()
...
impl MemoryLayer for PersonalityGuard {
    ...
    async fn process(&self, context: &mut Context) -> anyhow::Result<()> {
        ...
        let has_persona = context.items.first().is_some_and(|i| {
            i.role == MemoryRole::System && i.content.contains(&self.core_persona)
        });
