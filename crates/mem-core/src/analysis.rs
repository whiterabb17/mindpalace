use crate::{RelevanceAnalyzer, ImportanceAnalyzer, MemoryItem, Context, LlmClient, MemoryRole};
...
impl AnalysisTimeoutConfig {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(val) = std::env::var("MINDPALACE_KEYWORD_TIMEOUT_MS") {
            if let Ok(ms) = val.parse::<u64>() { config.keyword_analyzer_timeout = Duration::from_millis(ms); }
        }
        if let Ok(val) = std::env::var("MINDPALACE_HEURISTIC_TIMEOUT_MS") {
            if let Ok(ms) = val.parse::<u64>() { config.heuristic_analyzer_timeout = Duration::from_millis(ms); }
        }
        if let Ok(val) = std::env::var("MINDPALACE_VECTOR_TIMEOUT_SECS") {
            if let Ok(secs) = val.parse::<u64>() { config.vector_analyzer_timeout = Duration::from_secs(secs); }
        }
        if let Ok(val) = std::env::var("MINDPALACE_LLM_TIMEOUT_SECS") {
            if let Ok(secs) = val.parse::<u64>() { config.llm_analyzer_timeout = Duration::from_secs(secs); }
        }
        if let Ok(val) = std::env::var("MINDPALACE_FALLBACK_CHAIN_TIMEOUT_SECS") {
            if let Ok(secs) = val.parse::<u64>() { config.fallback_chain_timeout = Duration::from_secs(secs); }
        }
        if let Ok(val) = std::env::var("MINDPALACE_CIRCUIT_BREAKER_RESET_SECS") {
            if let Ok(secs) = val.parse::<u64>() { config.circuit_breaker_reset = Duration::from_secs(secs); }
        }
        config
    }
...
impl AnalyzerHealthMonitor {
    ...
    pub fn get_active_analyzer(&self) -> AnalyzerStrategy {
        ...
        if llm_failures >= self.timeout_threshold {
            ...
            tracing::warn!(vector_failures, "Vector analyzer degraded, using heuristic fallback");
    ...
    pub fn record_llm_timeout(&self) { self.llm_timeouts.fetch_add(1, Ordering::SeqCst); }
    pub fn record_llm_success(&self) { self.llm_timeouts.store(0, Ordering::SeqCst); }
    pub fn record_vector_timeout(&self) { self.vector_timeouts.fetch_add(1, Ordering::SeqCst); }
    pub fn record_vector_success(&self) { self.vector_timeouts.store(0, Ordering::SeqCst); }
}
...
pub struct LlmRelevanceAnalyzer { pub llm: Arc<dyn LlmClient> }
impl LlmRelevanceAnalyzer { pub fn new(llm: Arc<dyn LlmClient>) -> Self { Self { llm } } }
...
pub struct LlmImportanceAnalyzer { pub llm: Arc<dyn LlmClient> }
impl LlmImportanceAnalyzer { pub fn new(llm: Arc<dyn LlmClient>) -> Self { Self { llm } } }
...
impl KeywordMatcher {
    pub fn new() -> Self {
        Self {
            high_importance: Regex::new(r"(?i)\b(goal|decision|finally|fixed|constraint|sql|architecture|fact)\b").unwrap(),
            medium_importance: Regex::new(r"(?i)\b(todo|next|implement|refactor|check|bug|issue)\b").unwrap(),
...
impl RelevanceAnalyzer for KeywordRelevanceAnalyzer {
    async fn score_relevance(&self, item: &MemoryItem, context: &Context) -> anyhow::Result<f32> {
        if context.items.is_empty() { return Ok(0.5); }
...
impl RelevanceAnalyzer for CachedVectorRelevanceAnalyzer {
    async fn score_relevance(&self, item: &MemoryItem, context: &Context) -> anyhow::Result<f32> {
        ...
        if last_msg.is_empty() { return Ok(0.5); }
            ...
            AnalyzerStrategy::Full => {
                ...
                .await
                {
                    Ok(Ok(score)) => {
                        ...
                        self.metrics.llm_analyzer_latency_secs.observe(duration.as_secs_f64());
