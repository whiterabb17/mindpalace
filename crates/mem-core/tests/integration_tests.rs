...
async fn test_analyzer_strategy_degradation() {
    ...
    let health_monitor = Arc::new(mem_core::analysis::AnalyzerHealthMonitor::new(config.clone()));
    
    // Default is Full
    assert_eq!(health_monitor.get_active_analyzer(), mem_core::analysis::AnalyzerStrategy::Full);
    ...
    assert_eq!(health_monitor.get_active_analyzer(), mem_core::analysis::AnalyzerStrategy::VectorOnly);
}
...
async fn test_context_window_overflow() {
    ...
    let item = mem_core::MemoryItem { role: mem_core::MemoryRole::Assistant, content: "testing overflow bounds".to_string(), timestamp: 1, metadata: Default::default() };
...
async fn test_cache_ttl_expiration() {
    ...
    let item = mem_core::MemoryItem { role: mem_core::MemoryRole::Assistant, content: "test".to_string(), timestamp: 1, metadata: Default::default() };
