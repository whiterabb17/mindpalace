...
fn bench_keyword_analyzer(c: &mut Criterion) {
    ...
    c.bench_function("keyword_relevance_large_context", |b| {
        b.to_async(tokio::runtime::Runtime::new().unwrap()).iter(|| async {
            analyzer.score_relevance(black_box(&item), black_box(&large_context)).await.unwrap()
        })
    });
}
...
fn bench_heuristic_importance(c: &mut Criterion) {
    ...
    let item = mem_core::MemoryItem { 
        ...
        content: "Goal: Implement a highly scalable architecture and fix technical debt.".to_string(), 
        timestamp: 1, 
        metadata: Default::default() 
    };
    ...
    c.bench_function("heuristic_importance_regex", |b| {
        b.to_async(tokio::runtime::Runtime::new().unwrap()).iter(|| async {
            analyzer.score_importance(black_box(&item), black_box(&context)).await.unwrap()
        })
    });
}
